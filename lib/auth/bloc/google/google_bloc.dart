...
class GoogleBloc extends Bloc<GoogleEvent, GoogleState> {
  GoogleBloc() : super(GoogleInitial()) {
    on<GoogleEvent>((event, emit) async {
      emit(GoogleLoading());
      try{
        final response = await userRepository.registrationWithGoogle(userModel = userModel.copyWith(phoneNumber: event.phoneNumber, username: event.fullName, pic_url: "default"));
